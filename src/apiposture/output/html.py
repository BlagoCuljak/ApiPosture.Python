...
_CSS = (
    ":root{--panel:#fff;--border:#dbe3ee;--text:#1e293b;--muted:#64748b;"
    ...
    "border-bottom:1px solid var(--border);padding-bottom:12px;color:#0f172a;}"
    ...
    "border-radius:16px;padding:24px;transition:0.2s ease;"
    "box-shadow:0 6px 20px var(--shadow);}"
    ".summary-card:hover{transform:translateY(-2px);}"
    ".summary-card .label{color:var(--muted);font-size:14px;}"
    ".summary-card .value{font-size:34px;font-weight:700;"
    "margin-top:8px;color:#0f172a;}"
    "table{width:100%;border-collapse:collapse;margin-top:18px;"
    "margin-bottom:30px;border-radius:14px;"
    "box-shadow:0 6px 18px var(--shadow);}"
    "th{background:#eff6ff;color:#1e293b;text-align:left;padding:15px;"
    "font-size:14px;border-bottom:1px solid var(--border);}"
    "td{background:var(--panel);border-top:1px solid var(--border);"
    "padding:15px;vertical-align:top;}"
    "tr:hover td{background:#f8fbff;}"
    "code{background:#eef2ff;color:#1d4ed8;padding:4px 8px;"
    "border-radius:6px;font-family:Consolas,monospace;font-size:13px;}"
    ".finding{background:var(--panel);border:1px solid var(--border);"
    "border-left:6px solid var(--medium);border-radius:16px;"
    "padding:24px;margin-bottom:24px;transition:0.2s ease;"
    "box-shadow:0 6px 18px var(--shadow);}"
    ...
    ".severity-badge{display:inline-block;padding:5px 12px;"
    ...
    ".recommendation{margin-top:20px;background:#f8fafc;"
    ...
    ".success{padding:18px;border-radius:12px;background:#dcfce7;"
    "color:#166534;border:1px solid #86efac;font-weight:bold;}"
...
class HtmlFormatter(OutputFormatter):
    ...
    def format(self, result: ScanResult) -> str:
        ...
        lines.append(
            f'<div class="meta"><strong>Generated:</strong> {_e(now)} UTC</div>'
        )
        ...
        lines.append(
            ...
            f"<strong>Scan Path:</strong> "
            f"<code>{_e(str(result.scan_path))}</code></p>"
        )
        ...
        if has_any:
            ...
            for severity in severities:
                ...
                if count > 0:
                    ...
                    cap = severity.value.capitalize()
                    lines.append(
                        f'<li><span class="severity-badge severity-{_e(sev)}">'
                        f"{_e(cap)}</span>"
        ...
        if result.endpoints:
            ...
            lines.append(
                "  <thead><tr>"
                "<th>Route</th><th>Methods</th>"
                "<th>Classification</th>"
                "<th>Framework</th><th>Function</th>"
                "</tr></thead>"
            )
            lines.append("  <tbody>")
        ...
        if result.active_findings:
            ...
            for finding in result.active_findings:
                ...
                cap = finding.severity.value.capitalize()
                lines.append(f'<div class="finding {_e(sev)}">')
                lines.append(
                    f'  <span class="severity-badge severity-{_e(sev)}">'
                    f"{_e(cap)}</span>"
                )
                lines.append(
                    f"  <h3>[{_e(finding.rule_id)}] {_e(finding.rule_name)}</h3>"
                )
                route = _e(finding.endpoint.full_route)
                methods = _e(finding.endpoint.display_methods)
                lines.append(
                    f"  <p><strong>Endpoint:</strong> "
                    f"<code>{route} [{methods}]</code></p>"
                )
                loc = _e(str(finding.location))
                lines.append(
                    f"  <p><strong>Location:</strong> <code>{loc}</code></p>"
                )
            ...
            lines.append("<h2>Security Findings</h2>")
            lines.append(
                '<div class="success">&#x2705; No security findings detected!</div>'
            )
